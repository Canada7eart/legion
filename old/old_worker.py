...
class ConnectorThread(threading.Thread):
    ...
    def start(self):
        active_connections_emission = {}

        while True:
            loop_init = now_milliseconds()
            ips_we_want = {}

            with meta_rlock:
                ips_we_want = copy.copy(meta["nodes"])

            for ip_we_want_and_dont_have in ips_we_want.difference(ips_we_want):
                s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                conn = s.create_connection((host, port), timeout=0.5)
                
                if conn:
                    EmissionThread(conn, self.meta, self.meta_rlock, self.db, self.db_rlock).start()

            # make it so we aren't continually looping here
            time.sleep(math.max(3000, now_milliseconds() - loop_init)/1000.0)
...
if __name__ == '__main__':
    ...
    connector_thread = ConnectorThread(meta, meta_rlock, db, db_rlock)
